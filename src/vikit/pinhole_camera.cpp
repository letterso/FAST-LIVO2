...
cam2world(const double& u, const double& v) const
{
  ...
  else
  {
    ...
    pt_normalized.x() = (u - cx_) / fx_;
    pt_normalized.y() = (v - cy_) / fy_;
    Eigen::Vector2d pt_undistorted = pt_normalized;
    for (int i = 0; i < 8; i++) {
        double r2 = pt_undistorted.squaredNorm();
        double r4 = r2 * r2;
        double r6 = r4 * r2;
        double radial_distortion = 1.0 + d_[0] * r2 + d_[1] * r4 + d_[4] * r6;
        double dx = 2.0 * d_[2] * pt_undistorted.x() * pt_undistorted.y() + d_[3] * (r2 + 2.0 * pt_undistorted.x() * pt_undistorted.x());
        double dy = d_[2]  * (r2 + 2.0 * pt_undistorted.y() * pt_undistorted.y()) + 2.0 * d_[3] * pt_undistorted.x() * pt_undistorted.y();
        pt_undistorted.x() = (pt_normalized.x() - dx) / radial_distortion;
        pt_undistorted.y() = (pt_normalized.y() - dy) / radial_distortion;
    }

    xyz[0] = pt_undistorted.x();
    xyz[1] = pt_undistorted.y();
    xyz[2] = 1.0;
  }
